...
def threaded_augment_images(image_files):
    ...
    theta_tfx = np.random.choice(range(270))
    transformed_raw_image = gen.apply_transform(image, {'theta': theta_tfx})
    new_image =Image.fromarray(transformed_raw_image, 'RGB')
    new_image = new_image.resize((1024, 1024), Image.ANTIALIAS)
...
if __name__ == "__main__":
    ...
    raw_image_files = sorted(glob.glob(raw_images_dir + '*.jpg',
                                recursive=True))
    ...
    for i in range(10):
        ...
        indices =  [i for _ in range(len(raw_image_files))]
